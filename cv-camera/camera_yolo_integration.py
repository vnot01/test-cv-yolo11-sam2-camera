#!/usr/bin/env python3
"""
Camera + YOLO Integration Script
Integrates web camera with YOLO11 inference for real-time object detection
"""
...
CAMERA_YOLO_DIR = Path("/home/my/mycv/storages/images/output/camera_yolo")
RESULTS_DIR = Path("/home/my/mycv/storages/images/output/camera_yolo/results")
INFERENCE_LOG_DIR = Path("/home/my/mycv/storages/images/output/camera_yolo/results/inference")
DETECTION_IMAGES_DIR = Path("/home/my/mycv/storages/images/output/camera_yolo/results/images")
YOLO_SCRIPT = Path("/home/my/mycv/cv-camera/camera_yolo_inference.py")
...
def run_yolo_inference(image_path):
    ...
    try:
        ...
        # Run YOLO script with image path as argument
        result = subprocess.run([
            sys.executable, str(YOLO_SCRIPT), str(image_path)
...
def index():
    """Main page"""
    return render_template('camera_yolo.html')
...
def capture_and_infer():
    """Capture image and run YOLO inference"""
    global camera
    ...
    CAMERA_YOLO_DIR.mkdir(parents=True, exist_ok=True)
    ...
    def run_inference():
        success, output = run_yolo_inference(str(filepath))
    ...
    return jsonify({
        ...
        'filename': filename,
        'path': str(filepath),
        'message': 'Image captured and YOLO inference started'
    })
...
def create_html_template():
    """Create HTML template for camera + YOLO interface"""
...
<head>
    ...
    <title>Jetson Orin Nano - Camera + YOLO</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>🎥🤖 Jetson Orin Nano - Camera + YOLO</h1>
        ...
        <div class="controls">
            ...
            <button onclick="captureAndInfer()" class="yolo-button">🤖 Capture + YOLO</button>
    ...
    <script>
        ...
        function captureAndInfer() {
            log('🤖 Capturing image and running YOLO inference...');
            fetch('/capture_and_infer')
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        log(`✅ Image captured: ${data.filename}`);
                        log('🔍 YOLO inference started in background...');
                        alert('Image captured and YOLO inference started!');
                    } else {
                        log(`❌ Capture + YOLO failed: ${data.error}`);
                        alert('Error: ' + data.error);
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    log('❌ Capture + YOLO error');
                    alert('Error capturing image and running YOLO');
                });
        }
        ...
        updateCameraInfo();
        updateButtonStates(true);
        log('🚀 Camera + YOLO system ready');
    </script>
</body>
</html>
    """
    
    template_file = template_dir / "camera_yolo.html"
...
def main():
    ...
    print("🎥🤖 Jetson Orin Nano - Camera + YOLO Integration")
    print("=" * 60)
    ...
    print("=" * 60)
    print("📱 Access the camera + YOLO at:")
