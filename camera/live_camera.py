...
def generate_frames():
    ...
    while True:
        if not is_streaming or camera is None:
            time.sleep(0.1)  # Wait when camera is stopped
            continue
        ...
        if not ret:
            # Camera might be disconnected, wait and continue
            time.sleep(0.1)
            continue
...
<body>
    ...
    <script>
            ...
            fetch('/start')
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Refresh the video stream with new URL
                        const img = document.getElementById('video-stream');
                        img.src = '/video_feed?t=' + new Date().getTime();
                    } else {
        ...
        // Initialize button states (camera starts running by default)
        updateButtonStates(true);
    </script>
</body>
</html>
