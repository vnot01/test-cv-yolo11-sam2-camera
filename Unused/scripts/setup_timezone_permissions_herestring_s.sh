#!/bin/bash
# Setup timezone sync permissions using here string with -S flag

echo "Setting up timezone sync permissions..."

# Create sudoers file directly
echo "my" | sudo -S tee /etc/sudoers.d/timezone-sync > /dev/null <<< "my ALL=(ALL) NOPASSWD: /usr/bin/timedatectl set-timezone *"

# Set proper permissions
echo "my" | sudo -S chmod 440 /etc/sudoers.d/timezone-sync

# Add user to time group
echo "my" | sudo -S usermod -a -G time my
